  ...
  axios.get(`http://roryeagan.com:8082/${url}`, {
    ...
    .then(({ data }) => {
      ...
      store.get('travel').then((travel) => {
        ...
        data.forEach((alarm) => {
          const calcGoOff = alarm.time - (alarm.commuteData.routes[0].legs[0].duration.value*1000) - (alarm.prepTime*5*60*1000) - (alarm.postTime*5*60*1000) - (1000*60*userSettings.defaultSnoozeTime*userSettings.defaultSnoozes) - offSet;
          // Q + D new code vvvvv
